...
def create_epochs(raw, response_path, timestamps, sfreq):
    ...
    for _, row in response_df.iterrows():
        ...
        sample_index = np.argmin(
            np.abs(timestamps - event_time)
        )  # Find the closest timestamp sample
...
def main():
    ...
    epoch_dir = os.path.join("./epoch", latest_folder_name)
    if not os.path.exists(epoch_dir):
        os.makedirs(epoch_dir)
    
    epoch_path = os.path.join(epoch_dir, "epoch.fif")
